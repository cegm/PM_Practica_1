...
float errorIntegralLeft = 0;
float errorDerivativeLeft = 0;
...
float errorIntegralRight = 0;
float errorDerivativeRight = 0;
...
float computePID(String motor) {
  float lastErrorLeft = errorLeft;
  float lastErrorRight = errorRight;

  tocPID = millis();
  elapsedPID = tocPID - ticPID;

  errorLeft = desiredSpeedLeft - measuredSpeedLeft;
  errorRight = desiredSpeedRight - measuredSpeedRight;

  errorIntegralLeft = errorIntegralLeft + errorLeft*elapsedPID;
  errorIntegralRight = errorIntegralRight + errorRight*elapsedPID;

  errorDerivativeLeft = (errorLeft - lastErrorLeft)/elapsedPID;
  errorDerivativeRight = (errorRight - lastErrorRight)/elapsedPID;

  PIDOutputLeft = kPLeft*errorLeft + kILeft*errorIntegralLeft + kDLeft*errorDerivativeLeft;
  PIDOutputRight = kPRight*errorRight + kIRight*errorIntegralRight + kDRight*errorDerivativeRight;
