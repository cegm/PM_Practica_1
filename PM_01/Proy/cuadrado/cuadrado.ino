...
double ticRotation = 0;
double tocRotation = 0;
double elapsedRotation = 0;
double t = ((L*pi/2)/(2*r*(2*pi/60)*180))*1000*1.3; 
...
void loop() {
  if(count < 4) {
    Serial.println(" actual: " + String(elapsed));
    if(millis() - tic < 4200) {
    
      setVelocityDirection("forward", 120, "left");
      setVelocityDirection("forward", 118, "right");
...
void rotate() {
  tic = millis();
  while(!doneRotating) {
    Serial.println("objetivo: " + String(t) + " actual: " + String(millis() - tic));
    
    if(millis() - tic < t ) {
      //Serial.println("obj: " + String(t) + " ac: " + String(elapsedRotation));
      setVelocityDirection("backward", 255, "left");
      setVelocityDirection("forward", 255, "right");
    }
    else {
