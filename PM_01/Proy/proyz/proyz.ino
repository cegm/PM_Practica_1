...
double t = ((L*pi/2)/(2*r*(2*pi/60)*100))*1000*1.3; 
...
void rotate() {
  ticRotation = 0;
  t = ((L*pi/2)/(2*r*(2*pi/60)*100))*1000*1.3; 
  while(!doneRotating) {
    tocRotation = millis();
    elapsedRotation = tocRotation - ticRotation;
    Serial.println(String(elapsedRotation));
    
    if(elapsedRotation < t + 15000) {
